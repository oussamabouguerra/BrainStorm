 ...
 * @author mouad
 */
public class EditSimpleUserForm extends Form{

    public EditSimpleUserForm(Form previous) {
        setTitle("Modify My Account");
            ...
            public void actionPerformed(ActionEvent evt) {
                if(tfNom.getText().length()==0)
                {
                    Dialog.show("Alert","Please fill all the fields",new Command("OK"));
                         ...
                         if (ServiceSimpleUser.getInstance().editSimpleUser(su))
                         {Dialog.show("Success","Modification terminée!",new Command("OK"));
                             previous.showBack();}
                         else
                             Dialog.show("Error","Erreur Modification!",new Command("OK"));
